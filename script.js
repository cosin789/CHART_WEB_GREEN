...
async function loadCharts(selectedYear = "2025") {
    try {
        ...
        const baseUrl = 'https://script.google.com/macros/s/AKfycbyiKMlbkrBXdU3y5mfwBTCS4MKGPCDftuWT5kaUSFoU_D10HUaAOOC8F2nV6p7bAhx2/exec'; 
        const apiUrl = `${baseUrl}?year=${selectedYear}`;
        ...
        if (result.status === "success" && Array.isArray(result.data)) {
            ...
            window.updateDashboard = function(index) {
                ...
                document.getElementById('pieTitle').textContent = `สัดส่วนคะแนนแต่ละหมวดของ: ${selectedData.label} (ปี ${selectedYear})`;
